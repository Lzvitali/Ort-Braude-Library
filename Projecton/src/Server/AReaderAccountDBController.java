	...
	private static ObjectMessage checkIfExistID(ObjectMessage msg, Connection connToSQL) 
	{
		try 
		{
			...
			if(rs.next())
			{
				...
				{
					String massege=ACopyDBController.checkIfBookIsAvailableForBorrow(msg, connToSQL);
		...
		try 
		{
			...
			else
			{
				...
				for ( String ss : arrFreeSearch)
				{
					...
					while(rs.next())
					{
						ReaderAccount readerAccount=new ReaderAccount(rs.getString(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(8),rs.getString(7));
						readerAccount.setNumOfDelays(rs.getInt(9));
						result.add(readerAccount);
					}
				}
			}
